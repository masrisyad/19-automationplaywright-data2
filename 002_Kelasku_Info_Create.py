...
class TestConfig(LoginConfig):
    info_text: str = os.getenv("KELASKU_INFO_TEXT", "Test 2")
    wait_after_action_seconds: int = int(os.getenv("WAIT_AFTER_ACTION_SECONDS", "3"))
    screenshot_path: Path = Path(os.getenv("SCREENSHOT_PATH", "screenshot2_kelasku_info.png"))
    report_path: Path = Path(os.getenv("REPORT_PATH", "test_report_kelasku_info.pdf"))
...
class PdfReport:
    ...
    def _add_title(pdf: FPDF) -> None:
        ...
        pdf.cell(200, 10, txt="Kelasku Info Create Test Report", ln=True, align="C")
        pdf.ln(10)
...
async def run_kelasku_info_test(config: TestConfig) -> TestResult:
    result = TestResult()
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            await create_kelasku_info(page, config, result)
...
async def create_kelasku_info(page, config: TestConfig, result: TestResult) -> None:
    try:
        ...
        await open_create_info_form(page, config, result)

        rich_text_frame = page.frame_locator('iframe[title="Rich Text Area"]')
        await rich_text_frame.locator("html").click(timeout=config.timeout_ms)
        result.add_log("Clicked on the 'Rich Text Area' iframe.")

        await rich_text_frame.get_by_label("Rich Text Area. Press ALT-0").fill(
            config.info_text,
            timeout=config.timeout_ms,
        )
        result.add_log(f"Filled '{config.info_text}' into the Rich Text Area.")

        await page.get_by_role("button", name="Buat Info").click(timeout=config.timeout_ms)
        result.add_log("Clicked on 'Buat Info' button.")
    except TimeoutError:
        result.add_log("Failed to complete Kelasku info creation steps.", "FAILED")
        raise


async def open_create_info_form(page, config: TestConfig, result: TestResult) -> None:
    add_selectors = [
        ("button role 'plus-circle'", page.get_by_role("button", name="plus-circle")),
        ("button role 'google-plus'", page.get_by_role("button", name="google-plus")),
        ("Ant Design google-plus icon", page.locator("button:has(.anticon-google-plus)").first),
        ("SVG google-plus icon", page.locator("button:has(svg[data-icon='google-plus'])").first),
    ]

    try:
        await click_first_available(add_selectors, config.timeout_ms)
        result.add_log("Clicked on add info button.")
        return
    except TimeoutError:
        result.add_log("Add info button not found. Trying to delete existing info first.")

    await dump_kelasku_buttons(page, result)
    await delete_existing_info(page, config, result)
    await click_first_available(add_selectors, config.timeout_ms)
    result.add_log("Clicked on add info button after deleting existing info.")
...
async def dump_kelasku_buttons(page, result: TestResult) -> None:
    ...
    print("DEBUG: Buttons found on Kelasku page:")
    for button in buttons:
        print(button)

    result.add_log(f"Dumped {len(buttons)} buttons from Kelasku page for selector debugging.")


async def delete_existing_info(page, config: TestConfig, result: TestResult) -> None:
    trash_selectors = [
    ...
    await click_first_available(trash_selectors, config.timeout_ms)
    result.add_log("Clicked on delete/trash button.")
...
async def main() -> None:
    ...
    try:
        result = await run_kelasku_info_test(test_config)
    except TimeoutError as error:
